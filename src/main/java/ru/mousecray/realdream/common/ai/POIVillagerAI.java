package ru.mousecray.realdream.common.ai;
...
import ru.mousecray.realdream.common.values.DayValue;
import ru.mousecray.realdream.common.values.Poi;
import ru.mousecray.realdream.common.values.TradeTable;
import ru.mousecray.realdream.nbt.EntityVillagerNBTPipeline;
...
public class POIVillagerAI extends EntityAIBase {
    ...
    public POIVillagerAI(EntityVillager villager) {
        ...
        setMutexBits(3);
        tag = RealDreamNBT.get(villager).getVillagerPipe();
    }
            ...
            if (poi != null && poi.isPOIExist(villager.getEntityWorld())) {
                ...
                if (poi != null) {
                    if (tag.savePoi(poi)) {
                        ...
                        RealDream.LOGGER.error("Cannot save POI for villager at {}", villager.getPosition());
                    }
                } else {
                    RealDream.LOGGER.debug("No POI found near villager at {}", villager.getPosition());
                }
            }
        } else {
        ...
        for (int y = -1; y < 2; ++y) {
            for (int x = -halfRad; x < halfRad; ++x) {
                for (int z = -halfRad; z < halfRad; ++z) {
                    ...
                    VillagerRegistry.VillagerProfession prof  = RDVillagerRegistry.getProfessionForBlock(block);
