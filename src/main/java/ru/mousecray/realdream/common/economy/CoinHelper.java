package ru.mousecray.realdream.common.economy;
...
import ru.mousecray.realdream.common.event.RDEventPipeline;
...
import ru.mousecray.realdream.utils.RDMathUtils;
...
public class CoinHelper {
    ...
    public static void dropInWorld(World world, DamageSource cause, EntityLivingBase entity,
        ...
        if (normal != null && !normal.isEmpty()) {
            ...
            if (fullNormal.isPositive()) {
                ...
                displayCoins.forEach((type, value) -> {
                    ...
                    long[] groups = RDMathUtils.distributeNumber(value, i);
                    if (groups != null) {
                        for (long group : groups) {
                            if (group > 0) {
                                long[] stacks = RDMathUtils.distributeNumber(group, 64);
                                if (stacks != null) {
        ...
        if (specific != null && !specific.isEmpty()) {
            for (CoinValue spec : specific) {
                if (spec != null && spec.isPositive()) {
                    ...
                    int[] array = RDMathUtils.distributeNumber(spec.getAsInt(), i);
        ...
        if (resource != null && !resource.isEmpty()) {
            for (CoinValue res : resource) {
                if (res != null && res.isPositive()) {
                    ...
                    int[] array = RDMathUtils.distributeNumber(res.getAsInt(), i);
        ...
        if (other != null && !other.isEmpty()) {
            for (CoinValue oth : other) {
                if (oth != null && oth.isPositive()) {
                    ...
                    long[] array = RDMathUtils.distributeNumber(oth.getValue(), i);
        ...
        CoinDropEvent coinDropEvent = RDEventPipeline.instance().fireCoinDropEvent(
        ...
        for (CoinValue finalCoin : finalCoins) {
            ...
            if (isNew) RealDreamNBT.get(stack).getCoinPipe().setIsNew();
            capturedDrops.add(new EntityItem(world, entity.posX, entity.posY, entity.posZ, stack));
        }

        int i = ForgeHooks.getLootingLevel(entity, cause.getTrueSource(), RDDamageSources.ON_DROPPED_COIN);
        if (!ForgeHooks.onLivingDrops(entity, RDDamageSources.ON_DROPPED_COIN, capturedDrops, i, entity.recentlyHit > 0)) {
