...
public class CoinHandler {
    ...
    public void onMobDeath(LivingDeathEvent event) {
        ...
        if (event.getSource() == RDDamageSources.ON_DROPPED_COIN) return;
        ...
        if (bronze > 0) {
            bronze += RDRandomUtils.normalPercentFrom(rng, bronze, 25, 1);
        ...
        if (walletCap != null) {
            ...
            if (stack.getItem() instanceof IWallet) {
                ...
                if (bronze != null) {
                    bronze = bronze.plus(
                            CoinValue.create(
                                    RDRandomUtils.normalPercentFrom(rand, bronze.getValue(), 25, 1),
                ...
                for (SpecificCoinType value : SpecificCoinType.values()) {
                    ...
                    if (spec != null) {
                        spec = spec.plus(
                                CoinValue.create(
                                        RDRandomUtils.normalPercentFrom(rand, spec.getAsInt(), 25, 1),
                                        value)
                        );
                ...
                for (ResourceCoinType value : ResourceCoinType.values()) {
                    ...
                    if (res != null) {
                        res = res.plus(
                                CoinValue.create(
                                        RDRandomUtils.normalPercentFrom(rand, res.getAsInt(), 25, 1),
                                        value)
                        );
            ...
            if (walletStack.getItem() instanceof IWallet) {
                ...
                if (remain != null && remain.isPositive()) {
                    ...
                    if (remain.getType() instanceof NormalCoinType) {
                        ...
                        int[] groups = RDMathUtils.toGroupWithMax(remain.getValue(), 64);
                        if (groups != null) {
            ...
            if (!inventoryResult.isEmpty()) {
                ...
                if (!firstSimilar || (!normal.isEmpty() || !specific.isEmpty() || !resource.isEmpty() || !other.isEmpty())) {
                    CoinHelper.dropInWorld(
                            ...
                            RealDreamNBT.get(stack).getCoinPipe().loadIsNew(), false
                    );
                }
