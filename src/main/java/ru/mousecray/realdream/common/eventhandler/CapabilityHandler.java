...
import ru.mousecray.realdream.network.RDPacketFactory;
import ru.mousecray.realdream.registry.RDCapabilities;
import ru.mousecray.realdream.registry.RDPackets;

public class CapabilityHandler {
    @SubscribeEvent
    public void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof EntityPlayer) {
            RDCapabilities.INSTANCE.getInvsToSave().forEach(provider -> event.addCapability(provider.getName(), provider.createNew()));
        }
    }
    ...
    public void onPlayerClone(PlayerEvent.Clone event) {
        ...
        RDCapabilities.INSTANCE.getInvsToSave().forEach(provider -> {
    ...
    private void syncWalletInventory(EntityPlayerMP player) {
        ...
        if (walletCap != null) {
            ...
            RDPackets.INSTANCE.sendTo(RDPacketFactory.WALLET_SYNC(stack), player);
