...
public class EntityVillagerNBTPipeline {
    ...
    private final RealDreamNBT.RealDreamNBTEntity container;

    private EntityVillagerNBTPipeline(RealDreamNBT.RealDreamNBTEntity container) { this.container = container; }

    static EntityVillagerNBTPipeline get(RealDreamNBT.RealDreamNBTEntity base) {
    ...
    public boolean savePoi(Poi poi) {
        ...
        if (!saveBlock(tag, poi.getBlock())) {
            RealDream.LOGGER.error("POI block can't saving");
            return false;
        }
    ...
    public Poi loadPoi() {
        ...
        } else RealDream.LOGGER.error("Attempt to load non-existent POI");
        return null;
    }
