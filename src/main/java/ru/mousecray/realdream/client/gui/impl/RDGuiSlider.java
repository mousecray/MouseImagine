...
public class RDGuiSlider<T extends RDGuiSlider<T>> extends RDGuiPanel<T> {
    private final RDGuiButton<?> knob;
    ...
    private void fireClickEvent(RDGuiMouseClickEvent<?> original) {
        if (onClick != null) {
            RDGuiEventFactory.pushMouseClickEvent(clickEvent, self(), original.getMc(), original.getMouseX(), original.getMouseY());
            onClick.accept(clickEvent);
        }
    }

    private void fireDragEvent(RDGuiMouseDragEvent<?> original) {
        if (onDrag != null) {
            RDGuiEventFactory.pushMouseDragEvent(dragEvent, self(), original.getMc(),
    ...
    public void onClick(Consumer<RDGuiMouseClickEvent<T>> consumer) {
        onClick = consumer;
    }

    public void onDrag(Consumer<RDGuiMouseDragEvent<T>> consumer) {
        onDrag = consumer;
    }
