...
package ru.mousecray.mouseproject.core.common.economy;
...
import ru.mousecray.mouseproject.api.utils.MouseMath;
import ru.mousecray.mouseproject.core.common.economy.coin.NormalCoinType;
import ru.mousecray.mouseproject.core.common.event.CoinDropEvent;
import ru.mousecray.mouseproject.core.common.event.MPEventPipeline;
import ru.mousecray.mouseproject.core.nbt.MouseProjectNBT;
import ru.mousecray.mouseproject.core.registry.MPDamageSources;
import ru.mousecray.mouseproject.core.registry.MPSounds;
        ...
        if (normal != null && !normal.isEmpty()) {
            ...
            if (fullNormal.isPositive()) {
                ...
                displayCoins.forEach((type, value) -> {
                    ...
                    long[] groups = MouseMath.distributeNumber(value, i);
                    if (groups != null) {
                        for (long group : groups) {
                            if (group > 0) {
                                long[] stacks = MouseMath.distributeNumber(group, 64);
                                if (stacks != null) {
        ...
        if (specific != null && !specific.isEmpty()) {
            for (CoinValue spec : specific) {
                if (spec != null && spec.isPositive()) {
                    ...
                    int[] array = MouseMath.distributeNumber(spec.getAsInt(), i);
        ...
        if (resource != null && !resource.isEmpty()) {
            for (CoinValue res : resource) {
                if (res != null && res.isPositive()) {
                    ...
                    int[] array = MouseMath.distributeNumber(res.getAsInt(), i);
        ...
        if (other != null && !other.isEmpty()) {
            for (CoinValue oth : other) {
                if (oth != null && oth.isPositive()) {
                    ...
                    long[] array = MouseMath.distributeNumber(oth.getValue(), i);
