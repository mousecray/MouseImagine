...
public class IItemState {
    private static final   Map<Item, Map<Integer, IItemState>> cache = new SoftHashMap<>();
    ...
    public IItemState(@Nonnull Item item, int meta) {
        this.item = Objects.requireNonNull(item);
        this.meta = Math.max(meta, 0);
    }

    public IItemState(Item item)               { this(item, 0); }

    public static IItemState create(Item item) { return create(item, 0); }

    public static IItemState create(Item item, int meta) {
        if (RealDream.getInstance().getConfig().ITEM_STATE_CACHE.getBooleanAnyway()) {
            return cache
                    .computeIfAbsent(item, key -> new HashMap<>())
                    .computeIfAbsent(meta, key -> new IItemState(item, meta));
        } else return new IItemState(item, meta);
    }
    ...
    public IBlockState getBlock() {
        return IBlockState.create(Block.getBlockFromItem(item), meta);
    }
    ...
    public boolean isEqual(IItemState other)    { return other != null && (other == this || item == other.item && meta == other.meta); }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof IItemState)) return false;
        IItemState that = (IItemState) o;
